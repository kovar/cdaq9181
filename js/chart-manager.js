/**
 * ChartManager — 4-channel voltage/scaled value chart using Chart.js with time axis.
 */
...
const MAX_POINTS = 5000; // per channel — safety cap for high sample rates
...
export class ChartManager {
   ...
   * @param {number[]} channels — array of 4 values
   */
  addReading(channels) {
    const now = new Date();
    for (let i = 0; i < 4; i++) {
  ...
  /**
   * Update chart dataset labels (e.g. when scaling config changes).
   * @param {string[]} labels — array of 4 label strings
   */
  setLabels(labels) {
    for (let i = 0; i < 4; i++) {
  ...
  #prune(now) {
    ...
    for (const ds of this.#datasets) {
      ...
      let i = 0;
      while (i < ds.length && ds[i].x.getTime() < cutoff) i++;
      if (i > 0) ds.splice(0, i);
      // Hard cap for high sample rates
